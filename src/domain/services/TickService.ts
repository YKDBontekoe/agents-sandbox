import { GameState, ResKey } from '../entities/GameState';
...
export function runTick(state: GameState, proposals: Proposal[]): { state: GameState; crisis: Crisis | null } {
  const resources: Record<ResKey, number> = { ...state.resources } as any;

  for (const p of proposals) {
    const delta = p.predicted_delta || {};
    for (const key of Object.keys(delta)) {
      const value = Number(delta[key as ResKey] ?? 0);
      const cur = Number(resources[key as ResKey] ?? 0);
      resources[key as ResKey] = Math.max(0, cur + value);
    }
  }

  const buildings: Building[] = Array.isArray(state.buildings) ? (state.buildings as any) : [];
  ...
  const routes2 = Array.isArray(state.routes) ? (state.routes as any) : [];
  ...
  for (const b of buildings) {
    ...
    const def = (SIM_BUILDINGS as any)[typeId];
    if (!def) continue;
    ...
    for (const [k, v] of Object.entries(def.inputs)) {
      ...
      const cur = Number((resources as any)[k] ?? 0);
  ...
  if (routes2.length > 0) {
    const byId2 = new Map<string, any>((buildings as any).map((bb: any) => [String(bb.id), bb]));
    const MAX_ROUTE_LEN = 20;
    for (const r of routes2) {
      const a = byId2.get(String(r.fromId));
      const b = byId2.get(String(r.toId));
      if (!a || !b) continue;
  ...
  if (crisis) {
    for (const [key, value] of Object.entries(crisis.penalty)) {
      const k = key as ResKey;
      resources[k] = Math.max(0, Number(resources[k] ?? 0) + Number(value));
    }
  }
