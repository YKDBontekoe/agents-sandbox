...
export class RunTickCommand implements Command {}

export interface RunTickResult {
  state: any
  crisis: null | { type: 'unrest' | 'threat'; message: string; penalty: Record<string, number> }
}

export class RunTickHandler implements CommandHandler<RunTickCommand, RunTickResult> {
  async execute(): Promise<RunTickResult> {
    ...
    let crisis: RunTickResult['crisis'] = null
