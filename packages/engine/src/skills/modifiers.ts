...
export function createEmptySkillEffects(): AccumulatedSkillEffects {
  return accumulateEffects([]);
}

export interface SkillComputationOptions {
  seed?: number;
  tree?: SkillTree;
}

export function deriveSkillEffects(
  skillIds: readonly string[] | undefined,
  options: SkillComputationOptions = {},
): AccumulatedSkillEffects {
  const ids = Array.isArray(skillIds)
    ? Array.from(new Set(skillIds.map(id => String(id))))
    : [];
  if (ids.length === 0) {
    return createEmptySkillEffects();
  }
  const tree = options.tree ?? generateSkillTree(options.seed ?? 12345);
  const unlocked = tree.nodes.filter(node => ids.includes(node.id));
  return accumulateEffects(unlocked);
}
