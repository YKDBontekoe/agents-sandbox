export function generateMap(size: number, seed: number) {
...
export function generateMapChunk(chunkX: number, chunkY: number, chunkSize: number, seed: number) {
  function mulberry32(a: number) {
    return function() {
      let t = (a += 0x6d2b79f5)
      t = Math.imul(t ^ (t >>> 15), t | 1)
  ...
  const chunk: string[][] = Array.from({ length: chunkSize }, () => Array.from({ length: chunkSize }, () => 'grass'))
  ...
  if (rng() < resourceChance) {
    ...
    while (attempts < 20) {
      const rx = Math.floor(rng() * chunkSize)
      const ry = Math.floor(rng() * chunkSize)
      if (chunk[ry][rx] === 'grass' || chunk[ry][rx] === 'forest' || chunk[ry][rx] === 'desert') {
        const resources = ['iron', 'gold', 'mana_crystal', 'grain_patch']
        chunk[ry][rx] = resources[Math.floor(rng() * resources.length)]
        break
      }
      attempts++
    }
